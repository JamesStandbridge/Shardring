...
signal stage_started(level_index: int, map_id: StringName, required_threat_budget: float)
...
func force_complete_stage_for_tests() -> void:
	_survived_threat_budget = _required_threat_budget
...
func _on_danger_spawned(
	...
	if _survived_threat_budget >= _required_threat_budget:
		_make_exit_available()
...
func _start_current_stage(clear_active_dangers: bool) -> void:
	...
	stage_started.emit(_level_index, get_current_map_id(), _required_threat_budget)
	stage_progress_changed.emit(_survived_threat_budget, _required_threat_budget)
	DebugLog.info(
		&"Stage",
		(
			"started level=%d map=%s required_threat=%.1f"
			% [_level_index, get_current_map_id(), _required_threat_budget]
		)
	)
