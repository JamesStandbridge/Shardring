...
var _callout_timer_seconds: float = 0.0
var _callout_duration_seconds: float = 0.0

@onready var _bar: ProgressBar = get_node_or_null("Panel/HealthBar") as ProgressBar
@onready var _label: Label = get_node_or_null("Panel/HealthLabel") as Label
...
@onready var _peak_label: Label = get_node_or_null("PeakLabel") as Label
@onready var _callout_label: Label = get_node_or_null("CalloutLabel") as Label
...
func _process(delta: float) -> void:
	...
	if _callout_timer_seconds > 0.0:
		_callout_timer_seconds = maxf(_callout_timer_seconds - delta, 0.0)
		_update_callout()
...
func request_callout(text: String, color: Color, duration_seconds: float) -> void:
	if _callout_label == null or duration_seconds <= 0.0:
		return

	_callout_label.text = text
	_callout_label.modulate = color
	_callout_duration_seconds = duration_seconds
	_callout_timer_seconds = duration_seconds
	_update_callout()
...
func is_callout_visible() -> bool:
	return _callout_timer_seconds > 0.0 and _callout_label != null and _callout_label.visible


func get_callout_text() -> String:
...
func _update_stage_stats() -> void:
	...
	if _objective_label != null:
		...
		if _stage_controller.is_exit_available():
			exit_text = "  EXIT READY  RISK x%.2f" % _stage_controller.get_risk_bonus_multiplier()
		_objective_label.text = (
			"SHARDS %d / %d%s"
			% [
				_stage_controller.get_collected_shards(),
				_stage_controller.get_required_shards(),
				exit_text,
			]
		)


func _update_pressure_feedback() -> void:
	if _peak_label == null:
		return
	if _danger_director == null:
		_danger_director = get_node_or_null(danger_director_path) as DangerDirector
	if _danger_director == null:
		_peak_label.visible = false
		return

	_peak_label.visible = (
		_danger_director.get_pressure_phase() == DangerDirector.PressurePhase.PEAK
	)
...
func _update_callout() -> void:
	...
	var ratio := _callout_timer_seconds / maxf(_callout_duration_seconds, 0.001)
	_callout_label.visible = true
	_callout_label.scale = Vector2.ONE * lerpf(0.94, 1.08, ratio)
	var color := _callout_label.modulate
